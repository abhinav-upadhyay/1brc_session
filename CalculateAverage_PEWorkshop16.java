...
public class CalculateAverage_PEWorkshop16 {

    /**
     * We don't need to copy the location and temperature value bytes into temporary byte arrays before processing them.
     * We need to scan the byte arrays again later, which has the same cost as reading from the memory mapped pages of
     * the file, so the write to the byte arrays is a waste.
     */
    ...
    private static class Table {
        ...
        public void put(Scanner scanner, long locationOffset, int locationLength, int hashCode, short temperature) {
            int tableIndex = hashCode & TABLE_MASK;
            Row r = table[tableIndex];
            if (r == null) {
                byte[] locationCopy = new byte[locationLength];
                for (int i = 0; i < locationLength; i++) {
                    locationCopy[i] = scanner.getByte(locationOffset + i);
                }
            ...
            while (r != null) {
                if (r.hashCode == hashCode && bytesEquals(scanner, locationOffset, locationLength, r.location)) {
        ...
        private static boolean bytesEquals(Scanner scanner, long offset, int length, byte[] array) {
            if (length != array.length) {
                return false;
            }
            for (int i = 0; i < length; i++) {
                if (scanner.getByte(offset + i) != array[i]) {
                    return false;
                }
            }
            return true;
    ...
    private static Table readFile(Scanner scanner) {
        ...
        while (scanner.hasNext()) {
            long locationStartOffset = scanner.pos;
            while ((b = scanner.getByte()) != ';') {
                hashCode = hashCode * 31 + b;
            }
            int locationLength = (int) (scanner.pos - locationStartOffset - 1);
            short temperature = parseTemperature(scanner);
            table.put(scanner, locationStartOffset, locationLength, hashCode, temperature);
            hashCode = 1;
        }
        return table;
    }
